						...
						$('.dependencies').each(function()
							{
								...
								for (var i=0;i<items.length;i++)
								{
									...
									{
										str += '<option value="'+$.fbuilder.htmlEncode(items[i].name)+'" '+((items[i].name == $(this).attr("dvalue"))?"selected":"")+'>'+$.fbuilder.htmlEncode(items[i].name)+' (' + $.fbuilder.htmlEncode(items[i].title) + ') </option>';
									}
								}	
								$(this).html(str);
							});
