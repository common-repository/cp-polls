...
<div class="wrap">
<h1><?php echo esc_html($this->plugin_name); ?> - Report</h1>

<input type="button" name="backbtn" value="Back to items list..." onclick="document.location='admin.php?page=<?php echo esc_attr($this->menu_parameter); ?>';">

<div id="normal-sortables" class="meta-box-sortables">
 <hr />
 <h3>This report is for: <?php if ($this->item != 0) echo esc_html($myform[0]->form_name); else echo 'All forms'; ?></h3>
</div>
...
<form action="admin.php" method="get">
 ...
 <nobr>Search for: <input type="text" name="search" value="<?php echo esc_attr( !empty($_GET["search"]) ? $_GET["search"] : '' ); ?>" /> &nbsp; &nbsp; &nbsp;</nobr> 
 <nobr>From: <input type="text" id="dfrom" name="dfrom" value="<?php echo esc_attr( !empty($_GET["dfrom"]) ? $_GET["dfrom"] : '' ); ?>" /> &nbsp; &nbsp; &nbsp; </nobr>
 <nobr>To: <input type="text" id="dto" name="dto" value="<?php echo esc_attr( !empty($_GET["dto"]) ? $_GET["dto"] : '' ); ?>" /> &nbsp; &nbsp; &nbsp; </nobr>
 ...
 <span class="submit"><input type="submit" name="<?php echo esc_attr($this->prefix); ?>_csv" value="Export to CSV" /></span></nobr>
</form>
...
<div style="border:1px solid black;width:320px;margin-right:10px;padding:0px;float:left;"> 
 ...
 <div class="canvas" id="cardiocontainer1" style="margin-left:10px;">
  <canvas id="cardio1"  width="300" height="200" questions='[{"color":"#00f","values":[<?php echo esc_attr($daily_messages); ?>]}]'></canvas>
 </div>
...
<div style="border:1px solid black;width:330px;margin:0px;padding:0px;float:left;"> 
 ...
 <div class="canvas" id="cardiocontainer2" style="margin-left:10px;">
  <canvas id="cardio2"  width="312" height="200" questions='[{"color":"#00f","values":[<?php echo esc_attr($hourly_messages); ?>]}]'></canvas>
 </div>
...
<form action="admin.php#formrep" name="cfm_formrep" method="get">
 ...
 <input type="hidden" id="dfrom" name="dfrom" value="<?php echo esc_attr( !empty($_GET["dfrom"]) ? $_GET["dfrom"] : '' ); ?>" />
 <input type="hidden" id="dto" name="dto" value="<?php echo esc_attr( !empty($_GET["dto"]) ? $_GET["dto"] : '' ); ?>" />    
 ...
 <?php
  ...
  foreach ($fields as $item => $value)
      if ($item != 'notifyto')    
          $buffer = '<option value="'.esc_attr($item).'"'.($_GET["field"]==$item?' selected':'').'>'.esc_html($this->get_form_field_label($item,$form)).'</option>' . $buffer;
  echo $buffer;    
 ?>
 </select>
 <br /><br />
...
<div style="border:1px solid black;width:650px;margin-right:10px;padding:0px;float:left;"> 
 <div style="border-bottom:1px solid black;padding:5px;background:#ECECEC;color:#21759B;font-weight: bold;">
   Report of values for: <em><?php echo esc_html($this->get_form_field_label($_GET["field"],$form)); ?></em>
 </div>
 
<div style="padding:10px;">
<?php
  if (isset($fields[sanitize_key($_GET["field"])]))
  ...
  foreach ($arr as $item => $value)
  {
      ...
      echo '<div style="width:'.round($value/$total*$totalsize).'px;border:1px solid black;margin-bottom:3px;font-size:9px;background-color:#'.esc_attr($color_array[$count]).'">'.round($value/$total*100,2).'%</div>';       
      $count++;
